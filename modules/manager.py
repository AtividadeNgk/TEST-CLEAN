...
def delete_bot_permanently(bot_id):
    ...
    try:
        ...
        # Deleta pagamentos relacionados
        cursor.execute("DELETE FROM PAYMENTS WHERE bot = ?", (bot_id,))
        
        # Deleta usuários relacionados (se houver)
        # Você pode adicionar outras tabelas relacionadas aqui
        
        conn.commit()
        print(f"Bot {bot_id} deletado permanentemente")
        return True
